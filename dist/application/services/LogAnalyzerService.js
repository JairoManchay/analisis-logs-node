...
exports.LogAnalyzerService = void 0;
class LogAnalyzerService {
    ...
    filterByDNI(errors, dni) {
        return errors.filter(group => group.dnIs.some(d => d.includes(dni)));
    }
    filterByService(errors, service) {
        return errors.filter(group => group.service.toLowerCase().includes(service.toLowerCase()));
    }
    filterByRequestId(errors, requestId) {
        return errors.filter(group => group.requestId?.includes(requestId));
    }
    groupErrors(errors) {
    ...
    filterByEndpoint(errors, endpoint) {
        return errors.filter(group => group.endpoint?.toLowerCase().includes(endpoint.toLowerCase()) ||
            group.service?.toLowerCase().includes(endpoint.toLowerCase()) ||
            group.message?.toLowerCase().includes(endpoint.toLowerCase()));
    }
    filterByEnvironment(errors, environment) {
        return errors.filter(group => group.environment === environment.toUpperCase());
    }
}
exports.LogAnalyzerService = LogAnalyzerService;
//# sourceMappingURL=LogAnalyzerService.js.map